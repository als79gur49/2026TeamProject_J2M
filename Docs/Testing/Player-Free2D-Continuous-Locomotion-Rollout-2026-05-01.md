...
`GameplayRuntimeFeatureFlags.DefaultGameplayLocomotion` is the readiness default-on bundle for gameplay hosts and boundary inventory tests. As of legacy ordinary movement deprecation Phase 3, `GameplayRuntimeFeatureFlags.None` no longer authorizes covered player/enemy/Charge fallback. As of Phase 4, `GameplayRuntimeFeatureFlags.LegacyOrdinaryFallbackBaseline` also no longer authorizes player legacy ordinary fallback. As of Phase 5, the explicit baseline also no longer authorizes enemy ordinary fallback; Charge baseline fallback remains retained.
...
Default bundle adoption is not full legacy deletion. Phase 5 removes the player and enemy runtime fallback pilots only. The v3 deletion-readiness gate lives in `Docs/Testing/Legacy-Ordinary-Unit-Movement-Deprecation-Readiness-2026-05-01.md` and keeps `MoveEntity`, `MovementExpander`, retained grid transactions, glide retained fallback, and Charge explicit legacy baseline out of the deletion target.
