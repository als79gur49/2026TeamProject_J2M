...
Default bundle adoption is not full legacy deletion. Phase 4 removes the player runtime fallback pilot only. The v3 deletion-readiness gate lives in `Docs/Testing/Legacy-Ordinary-Unit-Movement-Deprecation-Readiness-2026-05-01.md` and keeps `MoveEntity`, `MovementExpander`, retained grid transactions, glide retained fallback, and enemy/Charge explicit legacy baseline out of the deletion target.
