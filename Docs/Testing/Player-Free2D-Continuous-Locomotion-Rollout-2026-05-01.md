...
- Boundary v1: `TickPipeline_ValidateLegacyExpansionIntents_BlocksFlagOnUnitOrdinaryMove`, `Replay_NoUnexpectedLegacyUnitOrdinaryMovementDetected`, and `Boundary_UnknownInventory_NormalGameplayHasNoUnexpectedUnknownMovement` verify that flag-on Free2D ordinary movement does not leak into legacy ordinary Unit movement while push/flip/item/topology grid transactions remain retained.
