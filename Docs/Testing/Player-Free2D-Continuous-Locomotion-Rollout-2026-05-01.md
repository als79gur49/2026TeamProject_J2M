...
`GameplayRuntimeFeatureFlags.DefaultGameplayLocomotion` is the readiness default-on bundle for gameplay hosts and boundary inventory tests. As of legacy ordinary movement deprecation Phase 3, `GameplayRuntimeFeatureFlags.None` no longer authorizes covered player/enemy/Charge fallback; intentional fallback tests use `GameplayRuntimeFeatureFlags.LegacyOrdinaryFallbackBaseline`.
...
Default bundle adoption is not legacy deletion. Phase 1 runtime validation is green for covered fallback isolation, and Phase 3 separates explicit fallback authorization from `None`, but actual deletion is not complete. The v3 deletion-readiness gate lives in `Docs/Testing/Legacy-Ordinary-Unit-Movement-Deprecation-Readiness-2026-05-01.md` and keeps `MoveEntity`, `MovementExpander`, retained grid transactions, glide retained fallback, and explicit legacy baseline out of the deletion target.
Scoped deletion preparation pins this branch with `ScopedDeletionPrep_PlayerLegacyFallback_IsFlagOffOnly` and `Replay_ScopedDeletionPrep_FlagOffFallbackStillDeterministic`; these tests preserve `None` fallback while asserting no covered fallback under `DefaultGameplayLocomotion`.
