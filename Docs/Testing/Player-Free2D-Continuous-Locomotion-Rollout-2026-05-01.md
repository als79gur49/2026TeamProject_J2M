...
Default bundle adoption is not legacy deletion. Phase 1 runtime validation is green for covered fallback isolation, but actual deletion is not complete. The v3 deletion-readiness gate lives in `Docs/Testing/Legacy-Ordinary-Unit-Movement-Deprecation-Readiness-2026-05-01.md` and keeps `MoveEntity`, `MovementExpander`, retained grid transactions, glide retained fallback, and flag-off baselines out of the deletion target.
