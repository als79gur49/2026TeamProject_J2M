...
- Stage-backed production overrides are `ActionAssistSettleWindowCells = 0.421875f` (`1728` fixed units) and `CollisionRadiusCells = 0.28125f` (`1152` fixed units). The assist window covers `84.375%` of a tile's full width, and the solid-front player-center clamp is `896` fixed units (`0.21875` cell) from the anchor center.
...
- Near-settled means `abs(localX) <= ActionAssistSettleWindowUnits && abs(localY) <= ActionAssistSettleWindowUnits`. The gate is inclusive: framework default tests queue at `512` and reject at `513`; stage-backed production queues at `1728` and rejects at `1729`.
...
- The actionable-candidate gate checks only whether current-anchor push/flip resolution has a candidate. It does not store a target entity at queue time. Framework-default composition keeps the `512` settle window; stage-backed production uses `1728`, so its `896` box-front radius clamp remains inside the Action Assist window.
...
- Framework-default regression coverage retains the historical `CollisionRadiusCells = 0.1875f` / `1280` clamp case outside the default `512` Action Assist window. Stage-backed production instead uses radius `0.28125f`, clamps at `896`, and must queue, align, and execute Push/Flip under the production `1728` window.
