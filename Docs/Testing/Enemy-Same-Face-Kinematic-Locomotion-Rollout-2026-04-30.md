...
`GameplayRuntimeFeatureFlags.DefaultGameplayLocomotion` enables this path for readiness canaries. As of legacy ordinary movement deprecation Phase 3, `None` is no longer the explicit legacy fallback baseline; after Phase 8B, removed-diagnostic compatibility tests use `GameplayRuntimeFeatureFlags.RemovedLegacyFallbackDiagnosticBaseline` without authorizing covered fallback. `GameplayRuntimeFeatureFlags.LegacyOrdinaryFallbackBaseline` remains only as a compatibility alias.
