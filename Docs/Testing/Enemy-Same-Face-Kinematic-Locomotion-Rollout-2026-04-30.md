...
- Boundary v1 stabilization adds `TickPipeline_legacy expansion validation hook (historical, deleted)_BlocksFlagOnUnitOrdinaryMove` and `Replay_NoUnexpectedLegacyUnitOrdinaryMovementDetected` canaries for enemy ordinary kinematic movement. The Phase 1 targeted Unity XML canaries are runtime green. Any flag-on enemy ordinary `Move` reaching `MovementExpander` is a regression.
...
`GameplayRuntimeFeatureFlags.DefaultGameplayLocomotion` enables this path for readiness canaries. As of legacy ordinary movement deprecation Phase 3, `None` is no longer the explicit legacy fallback baseline; after Phase 8B/8C, removed-diagnostic compatibility tests use the canonical `the removed diagnostic baseline preset (historical, deleted)` without authorizing covered fallback. Old diagnostic baseline alias vocabulary is historical-only and is not accepted by runtime code.
