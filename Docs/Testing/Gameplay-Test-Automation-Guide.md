...
- 현재 comic-sequence 전환 slice는 2026-08-19 KST에 기존 MP4/VideoPlayer 전용 런타임과 70-method legacy mixed suite를 제거하고 13개 공통 라우팅 테스트 및 확장된 만화 회귀 guard로 대체했다. 별도 아웃트로 Definition에는 표시 검증을 위해 인트로 콘텐츠 참조를 복제했다. Windows UI build와 Unity UI EditMode `1324 total / 0 failed`가 통과했으며, 이 수치는 위 pinned snapshot row를 대체하지 않는다.
...
- The current comic-sequence transition slice removed the MP4/VideoPlayer-only runtime and a 70-method legacy mixed suite on 2026-08-19 KST, replacing it with 13 shared-routing tests and expanded comic regression guards. A separate outro definition duplicates the intro content references for temporary presentation validation. The Windows UI build and Unity UI EditMode `1324 total / 0 failed` passed; this does not replace the pinned snapshot row above.
