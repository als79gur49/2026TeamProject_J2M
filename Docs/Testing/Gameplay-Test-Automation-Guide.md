...
- `./run_tests.sh gameplay-performance`
  ...
  - `GAMEPLAY_PERFORMANCE_STAGE_ID=stage-4-2` 또는 `stage-4-3`으로 동일한 capture를 해당 stage에서 실행할 수 있다. 허용되지 않은 stage 값은 실행 전에 거부한다.
  ...
  - Capture build는 gameplay-neutral Tick을 input preparation, simulation, host post-process, presentation, callback의 상호 배타적인 5개 구간으로 나눈다. schema 7 세부 probe는 Simulation을 7개 직접 자식, Plan과 Resolve를 각각 8개 직접 자식으로 나눈다. Plan의 pre-movement state/utility 구간은 setup, logic, bookkeeping, projection apply, utility input snapshot, utility resolve, utility projection apply로 추가 분해한다. Resolve의 초기 projection/BeforeAttack 구간은 4개 직접 자식으로 나누고, 그 안의 최초 post-movement snapshot은 base import/overlay apply/snapshot materialization으로, BeforeAttack AI transition은 setup/logic/projection apply로 추가 분해한다. 최초 snapshot은 조건부 Resolve rematerialization과 별도 reason으로 식별하고 Tick마다 정확히 한 번 기록되지 않으면 capture를 실패시킨다. Presentation은 coordinator/camera-state와 coordinator의 5개 직접 자식으로 나눠 `tick-attribution.json`에 기록한다. `tick-attribution-report.json`은 동일 revision/stage, 예상 샘플 수, 단일 thread, 연속 Tick index와 모든 중첩 부모-자식 시간 합을 fail-closed로 검증한다. 일반 Player에는 이 probe가 포함되지 않는다.
  - Stage별 Tick attribution 판정은 기존 performance 및 Cleanup S3 admission과 별도다. 특히 비기본 stage 진단에서 Cleanup S3가 HOLD여도 `tick-attribution-report.json`과 `performance-admission-report.json`의 verdict를 각각 확인한다. Attribution admission이 끝난 뒤 artifact manifest는 원본, report, validator의 SHA-256을 함께 기록하며, 최종 evidence transport는 세 파일의 현재 해시와 report 내부의 source/identity-source/validator binding을 다시 검증한다. 세 해시 중 일부만 존재하거나 어느 binding이라도 다르면 fail-closed다. Validator가 거부하면 attempt lifecycle은 `tickAttributionAdmission`을 `HOLD`로 종료하고 `HOLD_TICK_ATTRIBUTION_ADMISSION`과 validator exit status를 남긴다.
...
- `./run_tests.sh gameplay-performance`
  ...
  - Set `GAMEPLAY_PERFORMANCE_STAGE_ID=stage-4-2` or `stage-4-3` to run the same capture on that stage. Unsupported stage values are rejected before execution.
  ...
  - The capture build partitions each gameplay-neutral Tick into five disjoint sections—input preparation, simulation, host post-process, presentation, and callbacks. Schema 7 detail probes further partition Simulation into seven direct children and Plan and Resolve into eight direct children each. Plan's pre-movement state/utility section is split into setup, logic, bookkeeping, projection apply, utility input snapshot, utility resolve, and utility projection apply. Resolve's initial-projection/BeforeAttack section has four direct children; its initial post-movement snapshot is further split into base import, overlay apply, and snapshot materialization, while BeforeAttack AI transition is split into setup, logic, and projection apply. The initial snapshot uses a reason distinct from conditional Resolve rematerializations, and capture fails unless it is recorded exactly once per Tick. Presentation is partitioned into coordinator/camera-state plus five direct coordinator children before writing `tick-attribution.json`. `tick-attribution-report.json` fail-closes on revision/stage identity, expected sample count, single-thread execution, contiguous Tick indices, and exact arithmetic at every nested parent/child boundary. The probe is absent from ordinary Players.
  - Tick-attribution admission is separate from the existing performance and Cleanup S3 admissions. For non-default-stage diagnostics, inspect the verdicts in `tick-attribution-report.json` and `performance-admission-report.json` independently even if Cleanup S3 remains on HOLD. After attribution admission, the artifact manifest records SHA-256 values for the source, report, and validator; final evidence transport revalidates those files and the report's source, identity-source, and validator bindings. A partial hash set or any binding mismatch fails closed. Validator rejection terminally holds `tickAttributionAdmission` with `HOLD_TICK_ATTRIBUTION_ADMISSION` and records the validator exit status.
