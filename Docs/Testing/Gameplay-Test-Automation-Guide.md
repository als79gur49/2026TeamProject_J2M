...
- `UIAudioScene` gameplay shell 런타임 UI smoke가 필요할 때는 [GameplayShell-Manual-Runtime-Smoke-Plan.md](./GameplayShell-Manual-Runtime-Smoke-Plan.md)를 사용한다. 이 문서는 `StageLaunchContextStore`에 선택 `StageId`를 주입하는 canonical runtime integration의 수동 companion evidence를 정의하며, 자동화 lane을 대체하지 않는다.
...
- Use [GameplayShell-Manual-Runtime-Smoke-Plan.md](./GameplayShell-Manual-Runtime-Smoke-Plan.md) when a `UIAudioScene` gameplay shell UI smoke pass is needed with a selected `StageId` primed through `StageLaunchContextStore`; it is the manual companion for canonical runtime-integration evidence and does not replace the automated lanes.
