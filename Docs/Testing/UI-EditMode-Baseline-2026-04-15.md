...
- Current four-locale representative layout-boundary rerun: green on 2026-09-22 KST, Windows UI build passed and Unity UI EditMode `1528 total / 0 failed`; the existing 12-capture Settings/Pause/Main Menu fixture now rejects localized TMP overflow, glyph meshes outside authored text rectangles beyond a bounded 1.5-unit bearing tolerance, and glyph meshes outside the capture frame for en-US/ko-KR/ja-JP/zh-CN. The canonical 1920x1080 visual lane is configured for ja-JP/zh-CN core screens plus representative reserved-key/rebinding states; its first clean-revision run exposed an existing 1.1-unit en-US save-slot status bearing beyond the initial one-unit tolerance, so no visual PASS is claimed until the adjusted committed revision is rerun.
- Current Windows build result: `dotnet build Game.Feature.UI.Tests.csproj -c Debug` passed with `0` errors
- Current Unity UI EditMode: `1528 total / 0 failed`
- Baseline test result: command `./run_tests.sh ui`, result `1528 total / 0 failed`, failed tests `none`, failure category `none`, PR change pre-existing failure `no`
