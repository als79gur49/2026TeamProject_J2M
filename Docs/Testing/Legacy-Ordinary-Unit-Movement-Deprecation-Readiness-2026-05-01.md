...
This pass is `Scoped Fallback Branch Inventory & Test-Only Isolation Preparation`. Phase 6 now blocks player, enemy, and Charge covered fallback from the runtime path, including `GameplayRuntimeFeatureFlags.LegacyOrdinaryFallbackBaseline`. It does not delete `MoveEntity`, `MovementExpander`, retained grid transactions, or default/flag-off glide fallback.
...
| legacy Charge active fallback | `EnemyLogic.ResolveBaselineGroundLocomotion`, `TryBuildEnemyChargeKinematicStartPayload`, `TickResultBuilder.ShouldUseChargeMovePresentation` | removed from runtime, including `LegacyOrdinaryFallbackBaseline` | active Charge emits ordinary `Move` ignoring units -> validation rejects before legacy expansion commit | `UnitKinematicRuntimeState(MotionMode.Charge)` plus kinematic presentation | no, blocked with `ChargeLegacyFallbackRemovedFromRuntime` | Charge pilot complete | Charge replay/golden cleanup remains | `Phase6_LegacyOrdinaryFallbackBaseline_ChargeFallbackRemoved` | default charge kinematic canaries, `Phase3_None_NoPlayerEnemyChargeLegacyFallback` | Charge presentation/helper cleanup |
...
| legacy `TickEntityMotionKind.ChargeMove` generation | `TickResultBuilder.ShouldUseChargeMovePresentation` | active Charge without authoritative `MotionMode.Charge` pose | legacy Charge `MoveEntity` occurs while charge state is Active | Charge kinematic presentation track | no for covered runtime fallback after Phase 6 | no | `ChargeMove` owner and golden policy | historical charge fallback tests superseded | `ScopedDeletionPrep_ChargeLegacyFallback_RemovedByPhase6` | Phase 6 charge presentation cleanup |
...
| explicit fallback baseline | `GameplayRuntimeFeatureFlags.LegacyOrdinaryFallbackBaseline` | player, enemy, and Charge fallback authorization removed | explicit preset keeps legacy flag shape for replay/migration helpers while validation rejects covered fallback | player/enemy/Charge replacement paths | no covered fallback retained | no | replay/golden contract and preset cleanup approval | `Replay_Phase6_LegacyBaseline_PlayerEnemyChargeRemoved` | `Replay_Phase3_None_NoCoveredFallback` | preset/helper cleanup |
