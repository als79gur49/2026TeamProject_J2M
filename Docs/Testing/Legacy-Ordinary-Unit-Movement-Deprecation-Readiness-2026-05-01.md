...
Phase 2, `Player Legacy Ordinary Fallback Pilot`, scoped deletion readiness to the player ordinary fallback branch only. It pinned the player source, flag reachability, leak guard, `MovementExpander` legacy candidate, `LegacyFallback` boundary, and legacy `TickEntityMotionKind.Move` presentation without deleting the branch. Phase 3 moved that fallback authorization from `GameplayRuntimeFeatureFlags.None` to `the removed diagnostic baseline preset (historical, deleted)`.

Phase 2B, `Enemy Ordinary Fallback Pilot`, scoped deletion readiness to the enemy ordinary fallback branch only. It pinned the enemy source, enemy kinematic flag reachability, leak guard, `MovementExpander` legacy candidate, `LegacyFallback` boundary, and legacy `TickEntityMotionKind.Move` presentation without deleting the branch. Phase 3 moved that fallback authorization from `GameplayRuntimeFeatureFlags.None` to `the removed diagnostic baseline preset (historical, deleted)`.

Phase 2C, `Charge Active Fallback Pilot`, scoped deletion readiness to the Charge active fallback branch only. It pinned the active Charge source, Charge kinematic flag reachability, leak guard, legacy active-step consumption, `LegacyFallback` boundary, and legacy `TickEntityMotionKind.ChargeMove` presentation without deleting the branch. Phase 3 moved that fallback authorization from `GameplayRuntimeFeatureFlags.None` to `the removed diagnostic baseline preset (historical, deleted)`.

Phase 3, `Explicit Legacy/Test-Only Fallback Policy`, is complete for runtime/test contract. `GameplayRuntimeFeatureFlags.None` no longer authorizes player ordinary, enemy ordinary, or Charge active covered fallback. Those fallback paths require `the removed diagnostic baseline preset (historical, deleted)`, and unauthorized attempts are blocked with `LegacyOrdinaryFallbackRequiresExplicitBaseline`.

Phase 4, `Player Fallback Removal Pilot`, removes player ordinary fallback authorization even from `the removed diagnostic baseline preset (historical, deleted)`. Player attempts under that preset are rejected with `PlayerLegacyFallbackRemovedFromRuntime`. At Phase 4, enemy ordinary fallback and Charge active fallback were still explicitly retained under the baseline.

Phase 5, `Enemy Fallback Removal Pilot`, removes enemy ordinary fallback authorization even from `the removed diagnostic baseline preset (historical, deleted)`. Enemy attempts under that preset are rejected with `EnemyLegacyFallbackRemovedFromRuntime`. At Phase 5, player ordinary fallback remained removed and Charge active fallback was still explicitly retained under the baseline.

Phase 6, `Charge Fallback Removal Pilot`, removes Charge active fallback authorization even from `the removed diagnostic baseline preset (historical, deleted)`. Charge attempts under that preset are rejected with `ChargeLegacyFallbackRemovedFromRuntime`. Player and enemy fallback remain removed, flag-off glide fallback remains a retained exception, and retained grid transactions remain allowed.

Phase 7, `Fallback Cleanup Readiness`, keeps `the removed diagnostic baseline preset (historical, deleted)` as a diagnostic compatibility preset only. Phase 8A, `Obsolete Helper / Test Naming Cleanup`, aligns helper, test, and documentation vocabulary with that removed-diagnostic policy while keeping runtime boundary behavior unchanged. The C안 final cleanup deletes the obsolete covered-fallback helper wrappers; current tests use canonical `Assert*Removed*` helpers.

Phase 8B, `Diagnostic Baseline Rename Readiness`, added `the removed diagnostic baseline preset (historical, deleted)` as the canonical preset name for deterministic removed diagnostics. The old `GameplayRuntimeFeatureFlags.RemovedDiagnosticBaselineAlias` alias is now removed; runtime boundary behavior is unchanged.

Phase 8C, `Legacy Diagnostic Alias Usage Cleanup Readiness`, migrated current internal tests, replay helpers, and current-policy documentation to `the removed diagnostic baseline preset (historical, deleted)`. The old alias is removed and may only remain in historical documentation mentions; runtime boundary behavior is unchanged.

Phase 8D, `Diagnostic Flag / Helper Naming Cleanup Readiness`, added `GameplayRuntimeFeatureFlags.deleted legacy fallback diagnostic flag` as the canonical helper property for removed-fallback diagnostic routing. The old helper alias is removed; runtime boundary behavior is unchanged.

Phase 8E, `Canonical Diagnostics Field Migration`, completes C안 immediate deletion. `GameplayRuntimeFeatureFlags.deleted legacy fallback diagnostic flag` is the canonical runtime field, no old-name compatibility projection remains, and trace vocabulary uses `deleted legacy fallback diagnostic flag=`. Runtime boundary behavior, scene serialization exposure, retained grid transactions, and glide policy are unchanged.
...
| Phase 8A obsolete helper/test naming cleanup | complete for scoped canaries | canonical removed-diagnostic helper names are stable; obsolete helper wrappers are deleted |
| Phase 8B diagnostic baseline rename readiness | complete for scoped canaries | `removed diagnostic baseline preset (historical, deleted)` is canonical; `RemovedDiagnosticBaselineAlias` is removed |
| Phase 8C legacy alias usage cleanup readiness | complete for scoped canaries | current tests/docs use `removed diagnostic baseline preset (historical, deleted)`; old alias retained only in historical docs |
| Phase 8D diagnostic helper naming cleanup readiness | complete for scoped canaries | current runtime/tests/docs use `deleted legacy fallback diagnostic flag`; `RemovedDiagnosticHelperAlias` is removed |
| Phase 8E canonical diagnostics field migration | complete for scoped canaries | `deleted legacy fallback diagnostic flag` is the canonical runtime field; old-name API and trace projection are removed |
...
This pass is `Scoped Fallback Branch Inventory & Test-Only Isolation Preparation`. Phase 6 now blocks player, enemy, and Charge covered fallback from the runtime path. `the removed diagnostic baseline preset (historical, deleted)` is the current diagnostic preset for reproducing those removed diagnostics. It does not delete `MoveEntity`, `MovementExpander`, retained grid transactions, or flag-off glide fallback.
...
|---|---|---|---|---|---|---|---|---|---|---|
| player legacy ordinary fallback | `legacy expansion validation hook (historical, deleted)`, `TryResolveForbiddenLegacyUnitOrdinaryMovement`, `MovementExpander.ExpandMoveLike/ExpandMove`, `PlayerLogic.CollectMovementIntents` | removed from runtime; `removed diagnostic baseline preset (historical, deleted)` reproduces diagnostics | `PlayerLogic` emits ordinary `RawMovementIntent(Move)` -> `MoveIntent`; validation rejects before legacy expansion commit | `UnitContinuousLocomotionState` or player kinematic fallback | no, blocked with `PlayerLegacyFallbackRemovedFromRuntime` | player pilot complete | replay/golden cleanup remains | `Phase4_RemovedDiagnosticBaseline_PlayerFallbackRemoved` | `Phase3_None_NoPlayerEnemyChargeLegacyFallback`, default canaries | player presentation/helper cleanup |
| enemy legacy ordinary fallback | `TickPipeline.TryResolveForbiddenLegacyUnitOrdinaryMovement`, `IsEnemyKinematicStartParticipant`, `MovementExpander`, `EnemyLogic.ResolveBaselineGroundLocomotion` | removed from runtime; `removed diagnostic baseline preset (historical, deleted)` reproduces diagnostics; glide flag-off fallback is separate retained exception | enemy AI/script emits ordinary `Move` and is not handled by kinematic planner | enemy ordinary `UnitKinematicRuntimeState(MotionMode.Voluntary)` plus `TickKinematicMotionTrack` | no, blocked with `EnemyLegacyFallbackRemovedFromRuntime` | enemy pilot complete | replay/golden cleanup remains | `Phase5_RemovedDiagnosticBaseline_EnemyFallbackRemoved` | `Phase3_None_NoPlayerEnemyChargeLegacyFallback`, default canaries | enemy presentation/helper cleanup |
| legacy Charge active fallback | `EnemyLogic.ResolveBaselineGroundLocomotion`, `TryBuildEnemyChargeKinematicStartPayload`, `TickResultBuilder.TryResolveMotionKind` | removed from runtime; `removed diagnostic baseline preset (historical, deleted)` reproduces diagnostics; runtime builder no longer infers `ChargeMove` | active Charge emits ordinary `Move` ignoring units -> validation rejects before legacy expansion commit | `UnitKinematicRuntimeState(MotionMode.Charge)` plus kinematic presentation | no, blocked with `ChargeLegacyFallbackRemovedFromRuntime` | Charge pilot complete | Charge replay/golden cleanup remains | `Phase6_RemovedDiagnosticBaseline_ChargeFallbackRemoved` | default charge kinematic canaries, `Phase3_None_NoPlayerEnemyChargeLegacyFallback`, `ChargeMoveIsolation_RuntimeBuilder_DoesNotInferChargeMove` | Charge enum/consumer cleanup only after owner approval |
...
- `Legacy Fallback`: historical runtime path now removed for covered player/enemy/Charge fallback. `removed diagnostic baseline preset (historical, deleted)` is the canonical preset for deterministic removed diagnostics; `RemovedDiagnosticBaselineAlias` is removed alias vocabulary.
...
| phase relocation | retired in PR E | retired `PhaseThroughLockedTarget` payload/state residue | reserved compatibility only | no | no | none current | no current phase relocation hash/trace requirement | medium compatibility | Retired Synthetic Residue | do not revive as retained grid transaction | retired contract / residue scan | retired |
...
|---|---|---|---|---|---|---|---|---|
| player legacy ordinary fallback | `TickPipeline` movement validation/finalization path for player `Move` | removed from runtime; `removed diagnostic baseline preset (historical, deleted)` reproduces diagnostics | `UnitContinuousLocomotionState` / player kinematic fallback | no, blocked with `PlayerLegacyFallbackRemovedFromRuntime` | pilot complete | replay/golden cleanup unresolved | player default/`None` no-fallback, removed diagnostic baseline, player replay | cleanup candidate |
| enemy legacy ordinary fallback | `legacy expansion validation hook (historical, deleted)`, `BuildForbiddenLegacyUnitOrdinaryIntentIds`, `MovementExpander` ordinary Unit branch | removed from runtime; `removed diagnostic baseline preset (historical, deleted)` reproduces diagnostics; glide exception boundary remains separate | enemy ordinary `UnitKinematicRuntimeState` | no, blocked with `EnemyLegacyFallbackRemovedFromRuntime` | pilot complete | replay/golden cleanup unresolved | enemy default/`None` no-fallback, removed diagnostic baseline, enemy replay | cleanup candidate |
| legacy charge active fallback | `TickPipeline` charge movement path and historical synthetic Charge entity-motion fixtures | removed from runtime; `removed diagnostic baseline preset (historical, deleted)` reproduces diagnostics; runtime builder no longer infers legacy Charge entity motion | charge kinematic `MotionMode.Charge` plus charge presentation signal | no, blocked with `ChargeLegacyFallbackRemovedFromRuntime` | pilot complete; consumer deletion complete | charge replay/golden cleanup unresolved | charge active default/`None`/diagnostic/charge-kinematic/all-kinematic no-fallback, charge replay | historical docs only |
| stale ordinary Unit move test helper | `MovementExecutionOwnershipAssert` and scenario/replay helper callers | test-only | Phase 1 boundary helpers and explicit retained-grid assertions | no runtime baseline | no | helper callers still cover retained paths | helper usage inventory and targeted helper replacement tests | Phase 2 cleanup candidate |
...
| removed diagnostic baseline | `the removed diagnostic baseline preset (historical, deleted)` callers | player/enemy/Charge fallback authorization removed | diagnostic compatibility flag shape until preset cleanup | no covered fallback retained | no | rollback/golden contract | player/enemy/Charge removed baseline canaries | cleanup candidate |
...
| explicit fallback baseline green | complete after Phase 6 | player, enemy, and Charge fallback removed; `removed diagnostic baseline preset (historical, deleted)` retained for deterministic diagnostics |
