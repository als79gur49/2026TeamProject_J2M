...
| glide active/state-only lifecycle | glide ticking/target suppression plus current chase fallback | `EnemyGlideRuntimeState(Active/LandingPending/Recovery/Cooldown)` | state-only plus current `LegacyFallback` ordinary move under chase | yes, on current active chase fallback | yes, on current active chase fallback | legacy horizontal `TickEntityMotionKind.Move` plus additive `TickEnemyGlidePresentationSignal` height | glide state hash plus position when fallback moves; glide presentation excluded | high before deletion | Potential Legacy Ordinary Leak | decide suppression/kinematic policy before deleting ordinary fallback | `BoundaryInventory_Glide_ActiveLegacyOrdinaryLeak_IsDocumented`, `GlidePresentation_ComposesWithLegacyActiveMove` | do not delete ordinary fallback until resolved |
