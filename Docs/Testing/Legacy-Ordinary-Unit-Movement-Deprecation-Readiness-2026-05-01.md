...
Phase 8D, `Diagnostic Flag / Helper Naming Cleanup Readiness`, added `GameplayRuntimeFeatureFlags.RemovedLegacyFallbackDiagnosticsEnabled` as the canonical helper property for removed-fallback diagnostic routing. The old helper alias is removed; runtime boundary behavior is unchanged.

Phase 8E, `Canonical Diagnostics Field Migration`, completes C안 immediate deletion. `GameplayRuntimeFeatureFlags.RemovedLegacyFallbackDiagnosticsEnabled` is the canonical runtime field, no old-name compatibility projection remains, and trace vocabulary uses `RemovedLegacyFallbackDiagnosticsEnabled=`. Runtime boundary behavior, scene serialization exposure, retained grid transactions, and glide policy are unchanged.
...
| Phase 8D diagnostic helper naming cleanup readiness | complete for scoped canaries | current runtime/tests/docs use `RemovedLegacyFallbackDiagnosticsEnabled`; `RemovedDiagnosticHelperAlias` is removed |
| Phase 8E canonical diagnostics field migration | complete for scoped canaries | `RemovedLegacyFallbackDiagnosticsEnabled` is the canonical runtime field; old-name API and trace projection are removed |
