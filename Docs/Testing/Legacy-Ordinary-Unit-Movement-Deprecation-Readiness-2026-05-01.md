...
Phase 7, `Fallback Cleanup Readiness`, kept `GameplayRuntimeFeatureFlags.LegacyOrdinaryFallbackBaseline` as a diagnostic compatibility preset only. Phase 8A, `Obsolete Helper / Test Naming Cleanup`, aligns helper, test, and documentation vocabulary with that removed-diagnostic policy while keeping runtime boundary behavior unchanged. The 2026-05-29 cleanup removes obsolete covered-fallback `Allows*` wrappers after caller migration; internal tests should use canonical `Assert*Removed*` helpers.

Phase 8B, `Diagnostic Baseline Rename Readiness`, added `GameplayRuntimeFeatureFlags.RemovedLegacyFallbackDiagnosticBaseline` as the canonical preset name for deterministic removed diagnostics. `GameplayRuntimeFeatureFlags.LegacyOrdinaryFallbackBaseline` was a deprecated compatibility alias in that phase; runtime boundary behavior was unchanged.

Phase 8C, `Legacy Diagnostic Alias Usage Cleanup Readiness`, migrated current internal tests, replay helpers, and current-policy documentation to `GameplayRuntimeFeatureFlags.RemovedLegacyFallbackDiagnosticBaseline`. Runtime boundary behavior was unchanged.

Phase 8D, `Diagnostic Flag / Helper Naming Cleanup Readiness`, added `GameplayRuntimeFeatureFlags.RemovedLegacyFallbackDiagnosticsEnabled` as the canonical helper property for removed-fallback diagnostic routing. `GameplayRuntimeFeatureFlags.LegacyOrdinaryFallbackEnabled` was a deprecated compatibility alias, and `GameplayRuntimeFeatureFlags.EnableLegacyOrdinaryUnitFallback` remains the underlying compatibility field pending Phase 8E inventory; runtime boundary behavior is unchanged.
...
The 2026-05-29 alias cleanup removes the `GameplayRuntimeFeatureFlags.LegacyOrdinaryFallbackBaseline` and `GameplayRuntimeFeatureFlags.LegacyOrdinaryFallbackEnabled` aliases after confirming active runtime/tests use `RemovedLegacyFallbackDiagnosticBaseline` and `RemovedLegacyFallbackDiagnosticsEnabled`. `LegacyOrdinaryFallbackBaseline` alias has been removed. `LegacyOrdinaryFallbackEnabled` alias has been removed.
...
| Phase 8B diagnostic baseline rename readiness | complete for scoped canaries | `RemovedLegacyFallbackDiagnosticBaseline` is canonical; the old preset alias is historical |
| Phase 8C legacy alias usage cleanup readiness | complete for scoped canaries | current tests/docs use `RemovedLegacyFallbackDiagnosticBaseline` |
| Phase 8D diagnostic helper naming cleanup readiness | complete for scoped canaries | current runtime/tests/docs use `RemovedLegacyFallbackDiagnosticsEnabled` |
...
| `MovementExpander` | retained component | grid transactions still use it; removed-diagnostic baselines do not authorize covered fallback | protect grid branch canaries |
...
- `Legacy Fallback`: historical runtime path now removed for covered player/enemy/Charge fallback. `RemovedLegacyFallbackDiagnosticBaseline` is the canonical preset for deterministic removed diagnostics.
...
- `DeprecationPhase1_RemovedDiagnosticBaseline_PlayerEnemyChargeRemoved`
...
| explicit fallback baseline green | complete after Phase 6 | player, enemy, and Charge fallback removed; deterministic diagnostics now use `RemovedLegacyFallbackDiagnosticBaseline` |
