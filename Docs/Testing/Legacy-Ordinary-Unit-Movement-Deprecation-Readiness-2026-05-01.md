...
| glide start | `EnemyLogic.CommitGlideState` | `EnemyGlideRuntimeState(Windup/Active)` | state-only | no | no | state/signal only | enemy glide state hash included | low for start tick | State-only Special Candidate | none for start tick | `BoundaryInventory_Glide_NoLegacyOrdinaryMoveLeak` | keep state-only start canary |
| glide active/state-only lifecycle | glide ticking/target suppression plus current chase fallback | `EnemyGlideRuntimeState(Active/LandingPending/Recovery/Cooldown)` | state-only plus current `LegacyFallback` ordinary move under chase | yes, on current active chase fallback | yes, on current active chase fallback | state/signal plus `TickEntityMotionKind.Move` on active chase fallback | glide state hash plus position when fallback moves | high before deletion | Potential Legacy Ordinary Leak | decide suppression/kinematic policy before deleting ordinary fallback | `BoundaryInventory_Glide_ActiveLegacyOrdinaryLeak_IsDocumented` | do not delete ordinary fallback until resolved |
| glide end | recovery/cooldown/clear | `EnemyGlideRuntimeState` | state-only | no | no | state/signal only | glide state hash/clear | low | State-only Special Candidate | none before deletion | glide lifecycle tests | docs-only unless displacement appears |
...
- `BoundaryInventory_Glide_ActiveLegacyOrdinaryLeak_IsDocumented`
...
| glide has hidden displacement path | `BoundaryInventory_Glide_NoLegacyOrdinaryMoveLeak` covers state-only start; `BoundaryInventory_Glide_ActiveLegacyOrdinaryLeak_IsDocumented` records the current active fallback blocker |
