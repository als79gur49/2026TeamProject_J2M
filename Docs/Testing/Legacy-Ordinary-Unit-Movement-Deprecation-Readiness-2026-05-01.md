...
Phase 4, `Player Fallback Removal Pilot`, removes player ordinary fallback authorization even from `GameplayRuntimeFeatureFlags.RemovedLegacyFallbackDiagnosticBaseline`. Player attempts under that preset are rejected with `PlayerOrdinaryMoveRejectedBeforeLegacyExpansion`. At Phase 4, enemy ordinary fallback and Charge active fallback were still explicitly retained under the baseline.
...
Phase 7, `Fallback Cleanup Readiness`, keeps `GameplayRuntimeFeatureFlags.RemovedLegacyFallbackDiagnosticBaseline` as a diagnostic compatibility preset only. Phase 8A, `Obsolete Helper / Test Naming Cleanup`, aligns helper, test, and documentation vocabulary with that removed-diagnostic policy while keeping runtime boundary behavior unchanged. The C안 final cleanup deletes the obsolete covered-fallback helper wrappers; current tests use canonical `Assert*Removed*` helpers.

Phase 8B, `Diagnostic Baseline Rename Readiness`, added `GameplayRuntimeFeatureFlags.RemovedLegacyFallbackDiagnosticBaseline` as the canonical preset name for deterministic removed diagnostics. The old `GameplayRuntimeFeatureFlags.RemovedDiagnosticBaselineAlias` alias is now removed; runtime boundary behavior is unchanged.
...
| Phase 8B diagnostic baseline rename readiness | complete for scoped canaries | `RemovedLegacyFallbackDiagnosticBaseline` is canonical; `RemovedDiagnosticBaselineAlias` is removed |
...
| `deleted player Kinematic scenario tests` | 28 passed, 1 failed | Phase 1 named canaries are green; `PlayerSameFaceContinuousLocomotion_FlagOn_BoxBlocksOrdinaryMove` is tracked as non-Phase-1 expectation failure |
...
- `Legacy Fallback`: historical runtime path now removed for covered player/enemy/Charge fallback. `RemovedLegacyFallbackDiagnosticBaseline` is the canonical preset for deterministic removed diagnostics; `RemovedDiagnosticBaselineAlias` is removed alias vocabulary.
...
- Unity XML `TestResults/phase1-simulation.xml`: `BoundaryInventoryScenarioTests`, `MovementPhaseScenarioTests`, `PlayerContinuousLocomotionScenarioTests`, `deleted player Kinematic scenario tests`, and enemy ordinary/charge `EnemyAiScenarioTests` subset
