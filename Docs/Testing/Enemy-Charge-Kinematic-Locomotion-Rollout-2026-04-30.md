...
The ChargeMove producer isolation package removes runtime builder inference to legacy `ChargeMove`; only explicit synthetic presentation compatibility may construct it, and any current runtime `ChargeMove` output under default, `None`, diagnostic, charge-kinematic, or all-kinematic lanes is a regression.
