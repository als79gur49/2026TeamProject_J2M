...
Phase 2C of legacy ordinary Unit movement deprecation is a historical Charge active fallback pilot only. Phase 3 superseded its `None` baseline policy with `LegacyOrdinaryFallbackBaseline`, now a deprecated compatibility alias for `RemovedLegacyFallbackDiagnosticBaseline`, and Phase 6/7 superseded that baseline authorization with removed diagnostics. The later consumer deletion removes legacy Charge entity-motion compatibility without deleting `MoveEntity`, `MovementExpander`, retained grid transactions, or glide retained fallback.
