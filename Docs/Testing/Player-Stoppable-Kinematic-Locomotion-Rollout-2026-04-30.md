...
This rollout is guarded by `GameplayRuntimeFeatureFlags.retired player stoppable locomotion flag`.
It is effective only when `retired player same-face locomotion flag` is also enabled.
If `retired player Free2D gate` is enabled, player ordinary movement bypasses the Held/reverse/queue branch and uses `UnitContinuousLocomotionState`; this rollout remains the fallback when the free2D flag is off.
...
- Perpendicular input while held is queued in `PlayerControlState.retired queued turn state`, resumes the current segment forward, and tries the queued move on the next Plan tick after settled-zero.
...
Set `retired player stoppable locomotion flag` to false to restore automatic player kinematic continuation while keeping player kinematic locomotion enabled.
Set `retired player same-face locomotion flag` to false to return to the legacy discrete player movement baseline.
