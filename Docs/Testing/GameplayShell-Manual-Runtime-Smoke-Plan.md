...
- Record `TooltipPopup` as retired from the current popup vocabulary after PR-TT1 found no production caller.
...
- Failures can be misclassified if `TooltipPopup` retirement is mistaken for permission to remove the Settings display hover hint or restore removed Settings accessibility toggles.
...
## 6. TooltipPopup Retirement Rules
- `TooltipPopup` is not a current popup vocabulary member after PR-TT1 found no production caller.
- The gameplay shell smoke must not restore Settings tooltip on/off or large text toggle rows to create a tooltip entry point.
- Settings display hover hint remains a local inline pointer-hover affordance and does not use `PopupId.Tooltip`.
- Treat any `PopupId.Tooltip`, `TooltipPopup` prefab, catalog field, factory case, presenter, viewmodel, or view in the current runtime path as contract residue.
- Do not add scene-local helpers, alternate bootstrap objects, or artificial debug triggers to satisfy retired tooltip coverage.
