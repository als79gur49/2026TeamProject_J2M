...
- This smoke remains a runtime validation for the canonical gameplay shell in `Assets/Scenes/UIAudioScene.unity`.
- The selected stage is supplied through `StageLaunchContextStore` with a concrete `StageId`, such as `tutorial-scene`, instead of by opening a raw stage scene path.
...
- Preserve the gameplay-shell runtime focus in `UIAudioScene` with a primed `StageId` rather than converting this into an EditMode-only or architecture-redesign task.
...
- The `SettingsScreen` tooltip info icon can regress into a broken or missing authored affordance even though the `UIAudioScene` gameplay shell now expects a real player-facing tooltip path for the selected stage.
...
- The `UIAudioScene` gameplay shell with the selected `StageId` is expected to expose `TooltipPopup` through the SettingsScreen tooltip info icon beside the tooltip toggle row.
...
- Escalate tooltip coverage to blocker only when:
  - the canonical `SettingsScreen` tooltip path is missing or broken in the `UIAudioScene` gameplay shell for the selected `StageId`
...
- Tooltip representative coverage is acceptable only if the `SettingsScreen` tooltip info icon reaches `TooltipPopup` on the canonical path in `UIAudioScene` with the selected `StageId`.
