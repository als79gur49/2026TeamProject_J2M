...
Run the Editor exporter from a clean committed worktree and write output outside the repository. The wrapper holds the worktree index and current branch-ref locks throughout Unity execution, fails closed when staged, unstaged, or untracked changes are present, and revalidates HEAD/tree after Unity exits. Known Unity font-import normalization is restored only on an exact match; any other concurrent change is preserved and fails the run:
