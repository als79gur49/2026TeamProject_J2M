...
- current production gameplay concrete live source는 `PlayerControlStateLogic`의 player flip windup이다.
- retired enemy locked-target cross-through / phase relocation slots remain compatibility-only and are not current live Phased producers.
...
- source extension rule:
  ...
  - retired `EnemyPhaseRelocation` / `RetiredEnemyPreMovement` compatibility slots must not be treated as generalized phase action schema, default future owner precedent, or current phase principle.
...
- canonical rule:
  ...
  - retired enemy locked-target cross-through / phase relocation slots are compatibility-only and must not be used as generalized phase movement precedent.
...
- status: 2026-09-16 A의 Unit-only gameplay contract는 유지한다. B의 Direct-only ordered Unit cache는 stage-4-2/4-3 재계측에서 채택 이득을 입증하지 못해 CurrentPolicy에서 철회했고, 두 selector가 기존 ordered-entity span을 직접 필터하는 CurrentPolicy로 교체·검증했다.
  ...
  - clean B-stage evidence: detached `d05a47761`에서 cache/selector/BlackEye combined filtered Full EditMode `84/0`, EnemyLogic/Modifier/BlackEye `254/0`을 재현했다. core는 D-drive detached B worktree의 `/mnt/d/J2M/evidence/ordered-unit-cache-20260915/remeasure-preflight-clean-B-d05a47761/core`에서 다시 실행해 EditMode `290/0`, PlayMode `112 total / 108 passed / 4 skipped / 0 failed`를 확인했다. 보강 commit `0541831a6`의 clean detached cache/selector/BlackEye combined는 lazy-first-read isolation을 포함해 `85/0`이었다.
  ...
  - raw XML/log는 `/mnt/d/J2M/evidence/ordered-unit-cache-20260915` 아래 A selector/BlackEye, clean B combined/related, 보강 commit, corrected `remeasure-preflight-clean-B-d05a47761/core`, 공통-overlay A/B core 결과를 구분해 보존한다. 기존 `clean-B-d05a47761/core`는 project-path 귀속이 잘못되어 closure evidence에서 제외하며, scenario/replay `229/0` 결과의 raw artifact는 이 bundle에 포함되지 않는다.
...
- occupancy storage/query는 fresh-acquisition aggregate의 truth-source가 아니다.
  ...
  - 모든 Unit record 포함, non-Unit 배제, entity-ID 정렬, selected/rejection result 의미는 StrongContract다. candidate source와 자료구조는 이 observable contract를 보존하는 한 CurrentPolicy다.
  ...
  - gameplay-performance wrapper의 최종 상태는 16 run 모두 `HOLD_CLEANUP_ADMISSION`이다. 별도 Cleanup S3 calibration의 full-scan strategy expectation mismatch와 allocation counter probe `0`이 원인이므로, performance/tick-attribution `ADMITTED`와 전체 wrapper `HOLD`를 분리해 해석한다.
  - ordered-entity span CurrentPolicy(C)는 `/mnt/d/J2M/evidence/ordered-entity-span-20260916/remeasure-performance`에서 동일 runner/harness 및 측정 조건으로 stage-4-3/4-2 각 4회, 총 8회 재계측했다. 전부 performance와 schema-7 attribution은 `ADMITTED`였고, wrapper는 동일한 독립 Cleanup S3 사유로 `HOLD_CLEANUP_ADMISSION`이었다. C capture 내부 revision/worktree/runtime-tree/runner/harness identity는 일관됐다.
  - C EnemyAI Plan median 평균은 stage-4-3 `0.041962 ms`, stage-4-2 `0.028862 ms`로 과거 B 대비 `-28.80%`/`-33.19%`, A 대비 `-7.32%`/`+1.14%`였다. raw tick별 Plan+BeforeAttack 합산 median도 stage-4-3 `0.082800 ms`(A/B 대비 `-9.36%`/`-9.51%`), stage-4-2 `0.033100 ms`(A/B 대비 `+0.46%`/`-31.26%`)였다.
  - C는 B의 first-read Plan 회귀를 제거하며 대체로 A 수준을 회복했다. stage-4-3에서는 B의 후속 BeforeAttack cache-hit 이득을 반환하지만 결합 median은 더 낮고 결합 p95는 A 대비 `+0.87%`다. 따라서 C를 낮은 복잡도의 CurrentPolicy로 유지하고 B는 복구하지 않는다.
  - C 전체 Tick median의 과거 A 대비 차이(stage-4-3 `-14.64%`, stage-4-2 `-2.47%`)는 새 A control과 interleave되지 않은 후속 campaign이며 whole-Tick noise도 커서 system-level speedup으로 주장하지 않는다. 상세 결과와 비교 한계는 `/mnt/d/J2M/evidence/ordered-entity-span-20260916/remeasure-summary.md`를 따른다.
