...
Existing runtime-generated UI is not automatically a preferred precedent. `DemoStageControl` now follows the catalog-authored popup path; its former fixed-hierarchy and pointer-only migration debt is resolved and must not be reintroduced.
...
| `DemoStageControlPanelView` | `GameplayPopupPrefabCatalog` supplies the authored English operator-assist prefab. All six actions have visible focus, directional navigation, disabled-action recovery, pointer/Submit parity, and Cancel/Close behavior. | Resolved prefab-first and keyboard-accessibility debt. Keep serialized-reference fail-fast validation, catalog instantiation, exactly-once listener tests, and feature-local presentation-source lifetime tests. |
...
| Main Menu popup layer mount | `MainMenuUiFlowInstaller.EnsurePopupLayerView()` creates `MainMenuPopupLayer`, a technical Backdrop, and a Content mount. The Backdrop Button implements popup-policy consume/close behavior rather than an independent player action. Actual Main Menu Confirm content comes from the authored `ConfirmPopup.prefab`. | Runtime infrastructure, not prefab migration debt. Keep the technical shell action-free; review authored migration only if it gains designer-owned styling or independent actions. |
| Main Menu Confirm popup runtime | `GameplayPopupRuntimeFactory` instantiates the catalog-authored `ConfirmPopup.prefab`, then binds localization, typography, navigation, and lifetime. Pointer and Submit converge on the popup's Confirm/Cancel semantics. | Prefab-first runtime composition, not a runtime-generated fixed-shell exception or migration candidate. |
| Main Menu Settings overlay mount | `MainMenuSettingsOverlayController.EnsureOverlayLayer()` creates a modal ordering root, a technical pointer blocker, and a Content mount. The blocker is not a navigation action; the mounted Settings view owns navigation and Back/nested-state behavior. | Runtime infrastructure. Keep it as a bounded modal blocker/mount while it has no independent player-visible action or designer-owned presentation contract. |
| Shared Settings runtime | `MainMenuSettingsRuntime` and `GameplayScreenRuntimeFactory` both delegate to `SettingsScreenRuntimeBuilder`, which instantiates and validates the authored `SettingsScreen.prefab`. Runtime-added relay components are non-visual lifecycle infrastructure. | Existing prefab-first path, not remaining runtime-generated shell debt. `SettingsLayoutUtility` has no production caller and is not evidence that the current Settings hierarchy is runtime-built. |
| Comic sequence fixed shell and dynamic panels | Both production installers reference and instantiate the same canonical `ComicSequenceOverlay.prefab`. The prefab owns the fixed Background, page/final viewports, final image, black-fade layer, CanvasGroup, and AudioSource. `EnsureHierarchy()` fail-fast validates those authored references and never generates or repairs the fixed shell. `EnsurePanelPool()` separately creates and reuses the variable number of non-interactive panel Images required by `ComicSequenceDefinition`. Pointer click and UI Submit converge on `RequestAdvance()` exactly once; Cancel and lower-layer navigation remain blocked while the focus-free, unskippable sequence owns input. | Resolved prefab-first migration. Keep the fixed shell authored and shared by both production scenes, keep malformed/missing prefab references fail-fast, and retain the data-driven panel pool as bounded runtime infrastructure. Preserve direct pointer/Submit exactly-once, transition-state input suppression, Cancel blocking, and pool-reuse tests. |

The Demo Stage Control and Comic sequence fixed-shell migrations are resolved; only the Comic Definition-sized panel pool remains bounded runtime infrastructure. Main Menu popup mounting and Main Menu Settings overlay mounting are also classified runtime infrastructure, while the actual Demo, Confirm, and Settings hierarchies follow prefab-first composition. The resolved Settings language and Pause preview Close paths, plus the two explicit alternative focus treatments, must not be reported as missing keyboard support merely because their navigation ownership or visuals differ from the standard recipe.
