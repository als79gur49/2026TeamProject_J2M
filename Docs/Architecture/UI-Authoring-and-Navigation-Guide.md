...
| Comic sequence fixed shell and dynamic panels | Both production installers create a `ComicSequenceOverlayView` root when none is authored, and `EnsureHierarchy()` creates the fixed Background, page/final viewports, final image, black-fade layer, CanvasGroup, and AudioSource. `EnsurePanelPool()` separately creates a variable number of non-interactive panel Images from `ComicSequenceDefinition`. Pointer click and UI Submit converge on `RequestAdvance()`, but the direct-input, focus-free, unskippable interaction has no dedicated pointer/Submit exactly-once and Cancel contract test. | The fixed base shell is a prefab migration candidate. The data-driven panel pool is runtime infrastructure and should remain runtime-populated. No documented runtime-generation exception is approved for the fixed shell; retaining it requires an explicit owner, reason, supported-input record, and review/removal condition. |

The remaining actionable debt is therefore the Demo Stage Control migration and the Comic sequence fixed base-shell migration. Main Menu popup mounting and Main Menu Settings overlay mounting are classified runtime infrastructure, while the actual Confirm and Settings hierarchies already follow prefab-first composition. The resolved Settings language and Pause preview Close paths, plus the two explicit alternative focus treatments, must not be reported as missing keyboard support merely because their navigation ownership or visuals differ from the standard recipe.
