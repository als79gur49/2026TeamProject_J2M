...
The remaining actionable debt in this inventory is therefore the Demo Stage Control migration. The Comic sequence fixed base-shell migration is resolved; only its Definition-sized panel pool remains bounded runtime infrastructure. Main Menu popup mounting and Main Menu Settings overlay mounting are also classified runtime infrastructure, while the actual Confirm and Settings hierarchies follow prefab-first composition. The resolved Settings language and Pause preview Close paths, plus the two explicit alternative focus treatments, must not be reported as missing keyboard support merely because their navigation ownership or visuals differ from the standard recipe.
