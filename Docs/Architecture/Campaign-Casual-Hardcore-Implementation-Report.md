...
- `StageBackedGameplaySceneInstallerTests`: at the 2026-09-25 feature revision, the launch test initialized the actual host from the installer-produced configuration. Casual, Hardcore and then-supported noncampaign cases inspected the first WorldSnapshot and HUD query before any tick, authored entity/other-entity preservation, shared timing preset preservation, and resolved cooldown ticks. Main integration later removed the noncampaign stage case because stage gameplay now requires a valid campaign slot or handoff; the rejection and camera-only cases remain separate.
...
- StageBackedGameplaySceneInstallerBase and existing preset pipeline: validated launch state supplies HP; Casual copies only the initial array's Player, then applies 2-second cooldown after presets. Shared presets/assets and camera-only initialization are preserved. Stage gameplay without a valid campaign slot or handoff is rejected under the integrated main contract.
