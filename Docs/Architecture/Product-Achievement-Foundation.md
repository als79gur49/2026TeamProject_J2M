...
Version 1 remains readable inside the current profile schema under its original receipt contract: canonical `CompletedStageId`, a nonblank legacy `StageRunId`, and the legacy Objective clear-source value. Unknown receipt versions and invalid payloads fail closed. A present-null receipt is preserved as the distinct `PresentWithoutPayload` state without being promoted to a normal-completion fact. `CampaignCompleted` alone does not require a normal-completion receipt: Force Clear can complete the Campaign without an eligible Objective clear fact. Invalid receipts are not repaired, inferred, or automatically rewritten to version 2. This dual-read/new-write receipt policy is nested inside `CampaignProfileDocument.SchemaVersion = 3`; no PlayerPrefs campaign root is part of the production contract.
...
When a normal final-stage clear is eligible, `CampaignCompleted = true` and the first eligible receipt are assigned inside the same Campaign slot mutation and durable save invocation. A save failure leaves neither change durable. Force Clear can set `CampaignCompleted = true` without creating a normal-completion receipt. A valid existing receipt is preserved without overwrite, and an invalid existing receipt is retained but is not eligible and is not silently repaired. New Game starts with no receipt, Delete Slot removes that slot fact, and neither operation changes the product-global achievement document. Current schema-3 profiles that omit the optional field load with a null receipt. Profile schemas 1 and 2 and unsupported forward versions fail closed.
