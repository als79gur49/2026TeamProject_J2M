...
After the committed record is loaded, the stage integration evaluates the clear and threshold rules together and submits all matching product IDs through one `EarnBatch` call. Therefore a first normal `stage-1-2` clear at 25 or fewer combined uses creates one product-ledger save and one provider publication batch for both achievements.
...
After resolver acquisition, reconciliation obtains the canonical production Campaign store through `CampaignSaveCompositionProvider`, whose factory composes the canonical profile facade and recovery state before the read, and uses `LoadAllWithReport` rather than reading `profile.json` or backup files. Missing/empty profiles are a no-op; blocked or failed load states earn nothing. A backup-recovered canonical result is eligible. Version-1 and version-2 receipts must still name a serialized-sequence final Stage. Multiple valid slot receipts collapse to one product-ledger earn, and bare `CampaignCompleted` never earns.
