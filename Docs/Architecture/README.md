...
- Physical — current `Saves/profile.json`은 `CampaignProfileDocument`의 `SchemaVersion = 3`이다. 각 `Slots[]`의 `StageClearProfileSnapshot`은 `CampaignStageClearProfileDocument`이고 `Records[]`를 사용하며, record에는 `StageId`, `HasAttempted`, `HasCleared`, `ClearCount`, record-level `ProcessedStageRunIds : string[]`가 저장된다. profile snapshot에는 이와 별개인 profile-level `ProcessedStageRunIds : string[]`와 `ProcessedClearAttemptIds : string[]`가 저장된다. 이전 profile `SchemaVersion = 1/2`는 UnsupportedVersion으로 차단하며 원본과 backup을 보존한다.
...
- 과거 schema-3 `SaveSlotStore` DTO, campaign PlayerPrefs backend, active-slot PlayerPrefs backend, key 기반 DirectPlay context는 제거되었다. 현재 런타임 campaign 저장 구현은 별개의 schema-3 `CampaignProfileDocument`와 schema-1 local state를 사용한다.
...
- [Campaign-Casual-Hardcore-Mode-Design.md](./Campaign-Casual-Hardcore-Mode-Design.md)
  - proposed initial Casual/Hardcore implementation: shared three-slot modes, existing transition engine and synchronous saves, narrow HP/timing bootstrap changes, failure recovery through a new launch, file map and focused validation; P1–P4 confirmed: remaining HP persists across manual retry/menu/relaunch, initial invulnerability 2 seconds, common achievements in both modes, old profile formats unsupported; stronger recovery guarantees are deferred
- [Campaign-Casual-Hardcore-Implementation-Prompt.md](./Campaign-Casual-Hardcore-Implementation-Prompt.md)
...
- [Campaign-Modes-Main-Integration-Plan.md](./Campaign-Modes-Main-Integration-Plan.md)
  - main integration plan: pinned revisions and commit inventory, six conflict resolutions, auto-merge and policy-document repairs, sequential validation, and PR readiness gates
- [Campaign-Modes-Main-Integration-Validation-Report.md](./Campaign-Modes-Main-Integration-Validation-Report.md)
  - executed merge, staged-tree validation evidence, Player save-safety scope, and remaining Draft PR manual checks
...
- [Pre-Release-Save-Baseline-Policy.md](./Pre-Release-Save-Baseline-Policy.md)
  - current canonical truth for first-public `profile.json` schema 3 and `local-launch-state.json` schema 1, unsupported PlayerPrefs campaign compatibility, and exact-scope pre-release QA reset boundary
