...
- Jump start, airborne, and landing are classified as `UnitSpecialLocomotion`; phase relocation remains `ScriptedRelocation`; glide remains a state-only special movement candidate and future kinematic migration candidate. Forced motion and knockback are future special movement requirements until they have explicit runtime state.
