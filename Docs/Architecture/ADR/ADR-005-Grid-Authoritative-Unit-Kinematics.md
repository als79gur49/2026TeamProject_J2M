...
- Legacy ordinary Unit movement deletion remains blocked after Phase 1 isolation until glide policy and replay/golden migration policy are decided. The deletion draft phases in the readiness document are planning gates only and do not remove `MoveEntity`, `MovementExpander`, retained grid transactions, active glide retained fallback, or flag-off fallback baselines.
