...
- synthetic legacy-only migration, partial-state hard failure, invalid clip의 save-before-failure 및 재실행 idempotency를 유지한다.
...
- visual screenshot은 자동 runtime/asset contract를 대체하지 않으며, 수동 표본 결과는 evidence note로 분리한다.
