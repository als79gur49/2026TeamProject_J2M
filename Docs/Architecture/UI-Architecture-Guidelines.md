...
- `Non-Negotiable` UI SFX ownership is split between coordinator-owned flow outcome cues and screen-runtime-owned local widget cues. `ScreenTransitioned`, `PopupOpened`, and `PopupCompleted` remain mechanical lifecycle signals; the coordinator transaction/outcome layer is the only flow-cue trigger seam. The detailed classifier matrix, ownership table, and hidden `Ui` channel product policy live in [Audio-Architecture-Guidelines.md](./Audio-Architecture-Guidelines.md).
