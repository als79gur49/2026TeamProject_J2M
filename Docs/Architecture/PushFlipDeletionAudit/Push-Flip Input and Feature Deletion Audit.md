...
| P1 | REMOVED_ALIAS | `GameplayRuntimeFeatureFlags.LegacyOrdinaryFallbackBaseline` and `LegacyOrdinaryFallbackEnabled` | Tests/docs migrated to canonical `RemovedLegacyFallbackDiagnosticBaseline` / `RemovedLegacyFallbackDiagnosticsEnabled`; aliases deleted. |
| P1/P3 | DELETE_CANDIDATE_LEGACY_INPUT_COMPAT | `EnableLegacyOrdinaryUnitFallback` | Requires compatibility/API/replay owner decision; not Push-specific and currently test-governed. |
...
| 18 | Legacy ordinary fallback feature flags | `PARTIAL_REMOVED_ALIAS` | `LegacyOrdinaryFallbackBaseline` / `LegacyOrdinaryFallbackEnabled` aliases were removed; `EnableLegacyOrdinaryUnitFallback` remains intentionally retained pending API/replay owner decision. |
