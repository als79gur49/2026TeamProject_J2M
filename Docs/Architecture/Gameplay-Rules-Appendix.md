...
- fresh acquisition aggregation rule:
  ...
  - clean B-stage closure evidence: detached `d05a47761`에서 cache/selector/BlackEye combined filtered Full EditMode `84/0`, EnemyLogic/Modifier/BlackEye `254/0`, core EditMode `290 passed / 0 failed`, core PlayMode `112 total / 108 passed / 4 skipped / 0 failed`를 재현했다. 보강 commit `0541831a6`에서는 lazy-first-read isolation을 포함한 cache/selector/BlackEye combined `85/0`을 clean detached worktree에서 확인했다.
  ...
  - raw XML/log 보존 범위는 `/mnt/d/J2M/evidence/ordered-unit-cache-20260915/A/selector-blackeye`, `clean-B-d05a47761/selector-blackeye-cache`, `clean-B-d05a47761/related-targeted`, `clean-B-d05a47761/core`, `clean-hardened-0541831a6/selector-blackeye-cache`다. 앞서 실행한 scenario/replay `229/0` raw artifact는 이 bundle에 포함되지 않는다.
- scope boundary:
  ...
  - `WorldSnapshot`은 ordered Unit read가 실제로 요청된 snapshot에서만 cache를 만드는 lazy policy를 사용한다. query가 실행되지 않은 snapshot의 배열 할당과 정렬은 피하지만, Unit이 0이어도 getter가 호출되면 두 번의 entity dictionary 순회와 빈 배열 sort 호출은 수행한다.
  ...
  - `OrderedUnitsEnumeratedCount`는 각 cache access에서 consumer에게 제공한 Unit 수를 집계한다. miss build의 실제 `2 × 전체 entity 수` scan 비용을 나타내는 성능 counter가 아니다.
