...
- `Push`
  - 플레이어가 이동하려는 칸의 박스를 진행 방향으로 `Sliding` 상태에 넣는다.
  - `Sliding` 상태의 박스는 현재 틱에 1칸, 이후 틱들에도 매 틱 1칸씩 계속 전진한다.
- `Flip`
  - 플레이어 옆 1칸의 박스를 플레이어 반대편 1칸으로 옮긴다.
  - `Flip`은 앞벽 경계를 넘을 수 없다.
- `Destroy`
  - 단독 파괴 속성이 아니라 `Push` 진행이 막혔을 때 파괴를 허용하는 보조 속성이다.
  - 즉 `Push + Destroy` 조합일 때만 "더 밀 수 없으면 파괴"가 성립한다.
...
- 성공 시 박스는 그 틱에 1칸 이동하고 `Sliding` 상태가 된다.
- `Sliding` 상태의 박스는 이후 틱에도 같은 방향으로 1칸씩 계속 이동한다.
- stopper 후보:
  - 활성 면 경계 중 허용되지 않은 경계
  - 지형
  - 블로킹 엔티티
- 예외:
  - `Bottom <-> Front` 공유 경계에서는 같은 tick의 다음 1칸으로 연결되고, 이후 tick에도 슬라이드가 이어진다
- 실패 규칙:
  - 현재 tick의 다음 1칸으로 전진할 수 없고 대상 박스가 `Push + Destroy`면 파괴
  - `Destroy`가 없으면 단순 실패
- 슬라이딩 중인 박스가 이후 tick에 더 전진할 수 없을 때도 같은 실패 규칙을 적용한다.
...
- `Push` 성공 박스는 `Sliding` 상태를 유지하다가 더 이상 전진하지 못한 tick의 `Cleanup` 이후 정지 상태로 돌아간다.
