...
| KEEP_CANONICAL | `StageAudioDefinition`, `StageAudioRuntimeRequestSource`, `BgmRequestRouter` | Stage BGM companion path | Stage validator validates direct profile metadata; runtime source submits priority requests to the router; flow coordinator executes playback. | `*_Audio.asset` companions serialized from each stage entry | stage audio runtime source | `CampaignStageFlowTests`, `BgmFlowArchitectureTests`, `StageAudioDefinitionValidationTests` | High | Preserve; treat disconnects as integration issues, not deletion. |
