...
- The Comic runtime path has two distinct authoring classifications. The fixed `ComicSequenceOverlayView` base shell (`Background`, page/final viewports, final image, black-fade layer, `CanvasGroup`, and `AudioSource`) is authored once in canonical `ComicSequenceOverlay.prefab` and directly referenced by both production installers. Missing or malformed authored references fail fast; the view does not generate or repair fixed-shell objects. The variable non-interactive panel Image pool is sized and positioned from each `ComicSequenceDefinition`, reused across presentations, and remains bounded runtime infrastructure.
...
- Localization scalability uses the package-free `UI.ViewShared` production authority: `UiLocaleCatalog` owns canonical identity, raw autonym, stable order, exact aliases, and `Draft`/`ShipReady` lifecycle, while one `LocaleSelectionPolicy` governs explicit requests, persisted restore, initial Unity selection, and post-start Unity events. Structured sealed preference results distinguish missing, raw loaded data, ordinary failures, and contract-violating nulls; a guarded reporter exposes operation-specific failures without changing selection. Startup precedence is valid persisted canonical/alias, valid selected identity, then catalog default, with post-health canonical rewrite for alias/invalid loaded values. `UI.Composition` alone owns PlayerPrefs, production reporting, active Unity registration, immutable `LocaleOptionModel` projections, canonical Locale application, external-event restoration/persistence, and apply -> custom event -> save -> diagnostic ordering. Startup health exact-lookups only the selected canonical locale's Settings title and canonical Stage display-name descriptors, never the emergency-English fallback; complete registered ShipReady table/key coverage, registered Draft intersection zero, and uncatalogued registered-code zero are editor governance responsibilities. The package-free resolver uses no-op persistence defaults when omitted, treats supported codes only as membership, and exposes only policy-governed `TrySetLocale`. Settings owns unique-current matching, cycle/wrap, and post-request actual-current verification; its View renders the current raw autonym with resolver-current typography. Live transition health and Phase 9-17 transition/font-residency work remain future phases.
- Read-only string governance follows `UI.Application` requirement projection -> `UI.Composition.Editor` Unity snapshot adapter -> package-free `UI.ViewShared` validator. The projection consumes existing UI contracts/descriptors, the adapter reads registered Locales and real UI/Stage String Tables without mutating assets, and lifecycle-aware diagnostics block ShipReady drift while reporting Draft incompleteness as non-blocking. This editor validation path does not own runtime locale selection, font residency, Addressables loading, or authoritative UI presentation state.
