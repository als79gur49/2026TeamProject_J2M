...
- Localization scalability uses the package-free `UI.ViewShared` production authority: `UiLocaleCatalog` owns canonical identity, stable order, exact aliases, and `Draft`/`ShipReady` lifecycle, while one `LocaleSelectionPolicy` governs explicit requests, persisted restore, initial Unity selection, and post-start Unity events. `UI.Composition` alone snapshots active Unity registration and applies canonical Locale objects; `AvailableLocaleCodes` is the catalog-ordered registered ShipReady intersection. Settings option-model generalization, Locale assets, and later persistence redesign remain future phases.
