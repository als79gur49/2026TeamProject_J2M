...
- Localization scalability currently has a shadow-only package-free foundation in `UI.ViewShared`: `UiLocaleCatalog` models canonical identity, stable order, exact aliases, and `Draft`/`ShipReady` lifecycle, while `LocaleSelectionPolicy` proves selection behavior against injected registered codes. The production resolver, Settings option model, `AvailableLocaleCodes`, Locale assets, and locale persistence still use the established en-US/ko-KR path until a separately approved runtime migration.
