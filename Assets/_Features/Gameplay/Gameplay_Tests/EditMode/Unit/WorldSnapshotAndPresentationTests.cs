...
namespace Game.Feature.Gameplay.Tests.Unit
{
    public sealed class WorldSnapshotAndPresentationTests
    {
        ...
        [Test]
        [Category("Core")]
        public void MovementCommit_PositionAndFacingCommit_CreatesMotionTrack()
        {
            var sourceCell = new SurfaceCell(FaceId.Floor, 0, 0);
            var destinationCell = new SurfaceCell(FaceId.Floor, 1, 0);
            var presentationData = BuildMovementCommitPresentationData(sourceCell, destinationCell, Direction.Right);

            Assert.That(presentationData.EntityMotions.Count, Is.EqualTo(1));
            Assert.That(presentationData.EntityMotions[0].MotionKind, Is.EqualTo(TickEntityMotionKind.Move));
        }

        [Test]
        [Category("Core")]
        public void MovementCommit_RightMove_FacingAndMotionDirectionRight()
        {
            var sourceCell = new SurfaceCell(FaceId.Floor, 0, 0);
            var destinationCell = new SurfaceCell(FaceId.Floor, 1, 0);
            var presentationData = BuildMovementCommitPresentationData(sourceCell, destinationCell, Direction.Right);

            var motion = presentationData.EntityMotions.Single();
            Assert.That(motion.Direction, Is.EqualTo(Direction.Right));
            Assert.That(motion.DestinationFacing, Is.EqualTo(Direction.Right));
        }

        [Test]
        [Category("Core")]
        public void MotionTrack_SourceTargetMatchesPoseMutation()
        {
            var sourceCell = new SurfaceCell(FaceId.Floor, 0, 0);
            var destinationCell = new SurfaceCell(FaceId.Floor, 1, 0);
            var presentationData = BuildMovementCommitPresentationData(sourceCell, destinationCell, Direction.Right);
            ...
            var motion = presentationData.EntityMotions.Single();
            Assert.That(motion.SourceCell, Is.EqualTo(sourceCell));
            Assert.That(motion.DestinationCell, Is.EqualTo(destinationCell));
        }

        [Test]
        [Category("Core")]
        public void MotionTrack_OperationIdMatchesMovementResolution()
        {
            var sourceCell = new SurfaceCell(FaceId.Floor, 0, 0);
            var destinationCell = new SurfaceCell(FaceId.Floor, 1, 0);
            var presentationData = BuildMovementCommitPresentationData(
                sourceCell,
                destinationCell,
                Direction.Right,
                operationId: 137,
            ...
            var motion = presentationData.EntityMotions.Single();
            Assert.That(motion.OperationId, Is.EqualTo(137));
        }

        [Test]
        [Category("Core")]
        public void MovementCommit_NoMotionTrack_TeleportRegression()
        {
            var sourceCell = new SurfaceCell(FaceId.Floor, 0, 0);
            var destinationCell = new SurfaceCell(FaceId.Floor, 1, 0);
            var presentationData = BuildMovementCommitPresentationData(sourceCell, destinationCell, Direction.Right);
            ...
            Assert.That(
                presentationData.EntityMotions.Any(motion =>
                    motion.EntityId == 10 &&
                    motion.SourceCell == sourceCell &&
                    motion.DestinationCell == destinationCell),
                Is.True);
        }

        [Test]
        [Category("Core")]
        public void KinematicPresentationRecord_CreatesKinematicTrackWithoutDiscreteEntityMotion()
        {
            var anchorCell = new SurfaceCell(FaceId.Floor, 0, 0);
            var sourceOffset = KinematicOffset2.Zero;
            var destinationOffset = new KinematicOffset2(KinematicFixed.FromRaw(1024), KinematicFixed.Zero);
            var record = new KinematicPresentationRecord(
                entityId: 10,
                tickIndex: 1,
                operationId: 211,
                actionSequenceId: 17,
                modeBefore: MotionMode.Held,
                modeAfter: MotionMode.Voluntary,
                anchorCellBefore: anchorCell,
                anchorCellAfter: anchorCell,
                localOffsetBefore: sourceOffset,
                localOffsetAfter: destinationOffset,
                velocityBefore: KinematicVelocity2.Zero,
                velocityAfter: new KinematicVelocity2(KinematicFixed.FromRaw(1024), KinematicFixed.Zero),
                ...
                mutationKind: KinematicMutationKind.ReleaseHold,
                source: "KinematicRelease",
                reason: "ReleaseHold");

            var presentationData = BuildKinematicPresentationData(anchorCell, record);

            Assert.That(presentationData.EntityMotions, Is.Empty);
            var track = presentationData.KinematicMotionTracks.Single();
            Assert.That(track.EntityId, Is.EqualTo(10));
            Assert.That(track.OperationId, Is.EqualTo(211));
            Assert.That(track.ActionSequenceId, Is.EqualTo(17));
            Assert.That(track.SourceLocalOffset, Is.EqualTo(sourceOffset));
            Assert.That(track.DestinationLocalOffset, Is.EqualTo(destinationOffset));
            Assert.That(track.MutationKind, Is.EqualTo(KinematicMutationKind.ReleaseHold));
            Assert.That(track.Source, Is.EqualTo("KinematicRelease"));
        }
        ...
        [Test]
        [Category("Core")]
        public void SpawnOrRespawn_DoesNotUseMovementMotionTrackUnlessDedicatedPolicy()
        {
            var sourceCell = new SurfaceCell(FaceId.Floor, 0, 0);
            var destinationCell = new SurfaceCell(FaceId.Floor, 1, 0);
            var preMovementSnapshot = CreateWorldState(
            ...
            var presentationData = new TickPresentationDataBuilder().Build(
                new TickPresentationBuildContext(
                    ...
                    postMovementSnapshot,
                    postMovementSnapshot,
                    CreateMovementPhaseResultWithPresentationRecords(),
                    AttackPhaseResult.Empty,
                    CleanupFixtureFactory.None()));
        ...
        private static MovementPhaseResult CreateMovementPhaseResultWithPresentationRecords(
            params MovementPresentationRecord[] records)
        {
            return new MovementPhaseResult(
                Array.Empty<RawMovementIntent>(),
                Array.Empty<MoveIntent>(),
        ...
        private static TickPresentationData BuildMovementCommitPresentationData(
            SurfaceCell sourceCell,
            SurfaceCell destinationCell,
            Direction direction,
            int operationId = 137,
            int movementResolutionId = 137)
        {
            var preMovementSnapshot = CreateWorldState(
                new[]
                {
            ...
            var postMovementSnapshot = CreateWorldState(
                new[]
                {
                    CreateEntity(10, EntityType.Unit, destinationCell, direction),
                }).CreateSnapshot();
            var record = new MovementPresentationRecord(
                entityId: 10,
                tickIndex: 1,
            ...
            return new TickPresentationDataBuilder().Build(
                new TickPresentationBuildContext(
                    ...
                    postMovementSnapshot,
                    postMovementSnapshot,
                    CreateMovementPhaseResultWithPresentationRecords(record),
                    AttackPhaseResult.Empty,
                    CleanupFixtureFactory.None()));
        }
        ...
        {
            return BuildKinematicPresentationData(anchorCell, anchorCell, Direction.Right, Direction.Right, record);
        }
            ...
            KinematicPresentationRecord record)
        {
            var preMovementSnapshot = CreateWorldState(
                new[]
                {
                    CreateEntity(10, EntityType.Unit, sourceCell, sourceFacing),
                }).CreateSnapshot();
            var postMovementSnapshot = CreateWorldState(
                new[]
                {
                    CreateEntity(10, EntityType.Unit, destinationCell, destinationFacing),
                }).CreateSnapshot();
