    ...
    public sealed class FuzzDeterminismTests
    {
        ...
        public void Fuzz_CuratedSeeds_ProduceIdenticalPerTickTraceAndDumpsTwice()
        {
            ...
            for (var i = 0; i < SmokeSeeds.Length; i++)
            {
                ...
                CollectionAssert.AreEqual(
                    comparison.FirstRunFrames.Select(frame => frame.PlayerDamageDump).ToArray(),
                    comparison.SecondRunFrames.Select(frame => frame.PlayerDamageDump).ToArray());
                CollectionAssert.AreEqual(
                    comparison.FirstRunFrames.Select(frame => frame.EnemyActionDump).ToArray(),
                    comparison.SecondRunFrames.Select(frame => frame.EnemyActionDump).ToArray());
                CollectionAssert.AreEqual(
        ...
        public void ReplayArtifactWriter_WritesFirstDivergentTickArtifacts()
        {
            ...
            var artifact = new ReplayDivergenceArtifact(
                ...
                new[]
                {
                    new TickReplayFrame(1, "AAAAAAAAAAAAAAAA", "Trace-A-1", "E=10|Pos=(0,0)", "E=10|NextExplicitActionAllowed=0|Action=Push|ActionSeq=1|ActionDirection=Right|ActionTarget=30|Start=1|Execute=2|Recovery=2|Attempted=0", "<empty>", "<empty>", "<empty>", "Layer=Unit|Cell=(0,0)|E=10|Face=Floor", "<empty>", "Event-A-1"),
                    new TickReplayFrame(2, "BBBBBBBBBBBBBBBB", "Trace-A-2", "E=10|Pos=(1,0)", "E=10|NextExplicitActionAllowed=4|Action=None|ActionSeq=0|ActionDirection=None|ActionTarget=0|Start=0|Execute=0|Recovery=0|Attempted=0", "<empty>", "<empty>", "<empty>", "Layer=Unit|Cell=(1,0)|E=10|Face=Floor", "10", "Event-A-2"),
                },
                new[]
                {
                    new TickReplayFrame(1, "AAAAAAAAAAAAAAAA", "Trace-A-1", "E=10|Pos=(0,0)", "E=10|NextExplicitActionAllowed=0|Action=Push|ActionSeq=1|ActionDirection=Right|ActionTarget=30|Start=1|Execute=2|Recovery=2|Attempted=0", "<empty>", "<empty>", "<empty>", "Layer=Unit|Cell=(0,0)|E=10|Face=Floor", "<empty>", "Event-A-1"),
                    new TickReplayFrame(2, "CCCCCCCCCCCCCCCC", "Trace-B-2", "E=10|Pos=(2,0)", "E=10|NextExplicitActionAllowed=0|Action=None|ActionSeq=0|ActionDirection=None|ActionTarget=0|Start=0|Execute=0|Recovery=0|Attempted=0", "<empty>", "<empty>", "<empty>", "Layer=Unit|Cell=(2,0)|E=10|Face=Floor", "<empty>", "Event-B-2"),
                },
            ...
            AssertArtifactFileExists(artifactDirectoryPath, "first_run_enemy_actions.txt");
            AssertArtifactFileExists(artifactDirectoryPath, "second_run_enemy_actions.txt");
            ...
            StringAssert.Contains("E=10|NextExplicitActionAllowed=4|Action=None", File.ReadAllText(Path.Combine(artifactDirectoryPath, "first_run_player_control.txt")));
            StringAssert.Contains("E=10|NextExplicitActionAllowed=0|Action=None", File.ReadAllText(Path.Combine(artifactDirectoryPath, "second_run_player_control.txt")));
