...
namespace Game.Feature.Gameplay.Tests.PlayMode
{
    public sealed class ActualSceneBootstrapSmokePlayModeTests
    {
        ...
        private static IEnumerator RunCasualInputProbe(bool demoRelaunch)
        {
            ...
            void Record(string phase)
            {
                ...
                var state = $"{phase}|time={Time.realtimeSinceStartup:F3}|ticks={tickCount}|hp={player.hp}|cell={player.position}" +
                    ...
                    $"|deathBlocked={ProbeInputField(input, "_isPlayerDeathInputBlocked")}|bound={ProbeInputField(input, "_areActionsBound")}" +
        ...
        {
            var info = typeof(GameplayInputHost).GetField(field, BindingFlags.Instance | BindingFlags.NonPublic);
            Assert.That(info, Is.Not.Null, $"GameplayInputHost probe field {field} is missing.");
