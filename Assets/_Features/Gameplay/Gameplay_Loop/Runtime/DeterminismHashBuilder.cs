            ...
            for (var i = 0; i < enemyUtilityEntries.Count; i++)
            {
                ...
                for (var effectIndex = 0; effectIndex < entry.State.EffectStates.Count; effectIndex++)
                {
                    builder
                        ...
                        .Append(entry.State.EffectStates[effectIndex].activeStartTick).Append('|')
                        .Append(entry.State.EffectStates[effectIndex].activeEndTickExclusive).Append('|')
                        .Append(entry.State.EffectStates[effectIndex].activeOriginCell.face).Append('|')
                        .Append(entry.State.EffectStates[effectIndex].activeOriginCell.x).Append('|')
                        .Append(entry.State.EffectStates[effectIndex].activeOriginCell.y).Append('|')
            ...
            IReadOnlyList<ScheduledFlipResolutionSnapshotEntry> scheduledFlipResolutions)
        {
            ...
            for (var i = 0; i < scheduledFlipResolutions.Count; i++)
            {
                ...
                builder
                    ...
                    .Append(contact.ActionStartTick).Append('|')
                    .Append(contact.ActionVisualImpactTick).Append('|')
                    .Append(contact.FlipExecuteDelayTicks).Append('|')
                    .Append(contact.FlipInputLockDurationTicks).Append('|')
