...
namespace Game.Feature.UI.Tests
{
    public sealed class UiGovernanceDocumentationTests
    {
        ...
        public void GameplayTestAutomationGuide_UsesStageNineUiLanguage_AndRemovesStaleUiCounts()
        {
            ...
            Assert.That(guide, Does.Contain("green on 2026-07-22 KST"));
            Assert.That(guide, Does.Contain("Windows `dotnet build Game.Feature.UI.Tests.csproj -c Debug` passed with `0` errors"));
            Assert.That(guide, Does.Contain("Unity UI EditMode `872 total / 0 failed`"));
            Assert.That(guide, Does.Contain("CommandLine-20260722-210829"));
            Assert.That(guide, Does.Contain("CommandLine-20260720-194045` remains 51-count historical evidence"));
        ...
        public void UiBaselineNote_RecordsPhaseOneCanonicalDriftCorrections_AndDeletionProtections()
        {
            ...
            Assert.That(resultSection, Does.Contain("Current Unity UI EditMode: `872 total / 0 failed`"));
            Assert.That(resultSection, Does.Contain("result `872 total / 0 failed`, failed tests `none`, failure category `none`"));
            Assert.That(resultSection, Does.Contain("CommandLine-20260722-210829"));
            ...
            Assert.That(baseline, Does.Contain("explicit inspector-bound slot roots for current authored slots"));
            Assert.That(baseline, Does.Contain("`ChanceSlotView*` name fallback only as a safety net"));
