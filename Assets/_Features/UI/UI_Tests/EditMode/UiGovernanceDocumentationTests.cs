...
namespace Game.Feature.UI.Tests
{
    public sealed class UiGovernanceDocumentationTests
    {
        ...
        public void GameplayTestAutomationGuide_UsesStageNineUiLanguage_AndRemovesStaleUiCounts()
        {
            ...
            Assert.That(guide, Does.Contain("Unity UI EditMode `690 total / 0 failed`"));
        ...
        public void UiBaselineNote_RecordsPhaseOneCanonicalDriftCorrections_AndDeletionProtections()
        {
            ...
            Assert.That(resultSection, Does.Contain("Current Unity UI EditMode: `690 total / 0 failed`"));
            Assert.That(resultSection, Does.Contain("Baseline test result: command `./run_tests.sh ui`, result `690 total / 0 failed`, failed tests `none`, failure category `none`, PR change pre-existing failure `no`"));
